...
def build_input(subject, description, channel, rt, category, urgency):
    ...
    return (f"{text} [CHANNEL] {channel} [CAT] {category} "
            f"[RT] {rt} [URGENCY] {urgency}")
...
def build_dossier(ticket_id, assigned, inferred, rt, sat, confidence, keywords):
    ...
    severity_delta = f"{assigned} to {inferred}"
    kw_str = ", ".join(keywords) if keywords else "none detected"
    if i_num > a_num:
        analysis = (f"Ticket assigned '{assigned}' but signals indicate '{inferred}'. "
    ...
    return {
        ...
        "feature_evidence": [
            {"signal": "keyword", "value": kw_str, "weight": str(len(keywords))},
            {"signal": "resolution_time", "value": str(rt),
             "interpretation": "fast high severity" if rt <= 24 else "slow low severity"},
            {"signal": "satisfaction_score", "value": str(sat),
             "interpretation": "low serious issue" if sat <= 3 else "high minor issue"}
        ],
...
if page == "Single Ticket":
    ...
    with col1:
        ...
        description = st.text_area("Ticket Description",
                        "I have been unable to login for 2 days. Error 403. Urgent.",
                        height=120)
        category    = st.selectbox("Issue Category",
                        ["Technical", "Account", "Billing", "Fraud", "General Inquiry"])
    with col2:
        assigned_priority = st.selectbox("Assigned Priority",
                                ["Low", "Medium", "High", "Critical"])
        channel           = st.selectbox("Ticket Channel", ["Email", "Chat", "Web Form"])
        resolution_time   = st.number_input("Resolution Time (hours)",
                                min_value=1.0, max_value=120.0, value=24.0)
        satisfaction      = st.number_input("Satisfaction Score (1-5)",
                                min_value=1.0, max_value=5.0, value=3.0)
    ...
    if st.button("Analyze Ticket", type="primary"):
        ...
        text_input = build_input(subject, description, channel,
        ...
        if pred == 1:
            ...
            if mtype == "Hidden Crisis":
                ...
                st.warning(f"MISMATCH DETECTED — {mtype}")
            c1, c2, c3 = st.columns(3)
            c1.metric("Assigned Priority", assigned_priority)
            c2.metric("Inferred Severity", inferred)
            c3.metric("Confidence", f"{conf:.1%}")
            dossier = build_dossier(ticket_id, assigned_priority, inferred,
                                     resolution_time, satisfaction, conf, keywords)
            st.markdown("### Evidence Dossier")
            st.json(dossier)
        else:
            st.success("Priority is CONSISTENT")
            c1, c2, c3 = st.columns(3)
            c1.metric("Assigned Priority", assigned_priority)
            c2.metric("Inferred Severity", inferred)
            c3.metric("Confidence", f"{conf:.1%}")
...
# PAGE 2: BATCH CSV
    ...
    st.markdown("Upload a CSV file to analyze multiple tickets.")
    st.markdown("---")
    ...
    if uploaded:
        ...
        if st.button("Run Batch Analysis", type="primary"):
            if 'combined_text' not in df.columns:
                df['combined_text'] = (df['Ticket_Subject'].str.lower() +
                                       ' [SEP] ' +
            ...
            for i in range(0, len(inputs), BATCH_SIZE):
                ...
                status.text(f"Processing {min(i+BATCH_SIZE, len(inputs))}/{len(inputs)}")
            ...
            c3.metric("Mismatch Rate", f"{mismatches/total:.1%}")
            st.dataframe(df[['Ticket_ID', 'Ticket_Subject',
                              'Priority_Level', 'result', 'confidence']])
            csv = df.to_csv(index=False).encode()
            st.download_button("Download Results CSV", csv,
                                "sia_predictions.csv", "text/csv")
...
# PAGE 3: DASHBOARD
    ...
    if not os.path.exists(pred_file):
        ...
        total      = len(df)
        mismatches = int(df['predicted_mismatch'].sum())
        ...
        c3.metric("Consistent",     f"{consistent:,}")
        ...
        with col1:
            ...
            pc = df.groupby(['Priority_Level', 'result']).size().reset_index(name='count')
            fig1 = px.bar(pc, x='Priority_Level', y='count', color='result',
                           barmode='group',
                           color_discrete_map={'Mismatch': '#ef4444', 'Consistent': '#22c55e'})
            st.plotly_chart(fig1, use_container_width=True)

        with col2:
            st.markdown("### Mismatch by Channel")
            cc = df.groupby(['Ticket_Channel', 'result']).size().reset_index(name='count')
            fig2 = px.bar(cc, x='Ticket_Channel', y='count', color='result',
                           barmode='group',
                           color_discrete_map={'Mismatch': '#ef4444', 'Consistent': '#22c55e'})
            st.plotly_chart(fig2, use_container_width=True)

        st.markdown("### Confidence Score Distribution")
        fig3 = px.histogram(df, x='confidence', color='result', nbins=50,
                             color_discrete_map={'Mismatch': '#ef4444', 'Consistent': '#22c55e'})
        ...
        if 'Issue_Category' in df.columns:
            ...
            heat = df[df['result'] == 'Mismatch'].groupby(
                ['Issue_Category', 'Ticket_Channel']).size().reset_index(name='count')
            fig4 = px.density_heatmap(heat, x='Ticket_Channel', y='Issue_Category',
